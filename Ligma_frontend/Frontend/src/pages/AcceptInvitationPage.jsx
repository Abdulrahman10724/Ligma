...
export default function AcceptInvitationPage() {
  ...
  const { currentInvitation, publicLoading, error } = useSelector(
  ...
  const canRespond = isRecipient && invitation?.status === "Pending";

  const STATUS_MESSAGES = {
    Accepted: "You've already accepted this invitation.",
    Rejected: "You've already declined this invitation.",
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[color:var(--bg-primary)] px-4 text-[color:var(--text-primary)]">
      <div className="w-full max-w-lg rounded-2xl border border-[color:var(--border)] bg-[color:var(--bg-surface)] p-8 shadow-sm text-left">
        <h2 className="text-3xl font-bold text-[color:var(--accent)]">
        ...
        {error ? (
          <div className="mt-6 rounded-lg border border-[color:var(--danger)]/20 bg-[color:var(--danger)]/10 px-4 py-3 text-sm text-[color:var(--danger)]">
            {error}
          </div>
        ) : null}
        ...
        {invitation ? (
          <div className="mt-6 rounded-xl border border-[color:var(--border)] bg-[color:var(--bg-primary)] p-5">
            ...
            <h3 className="mt-1 text-xl font-semibold text-[color:var(--text-primary)]">
            ...
            {canRespond ? (
              ...
              <div className="mt-6 rounded-lg border border-[color:var(--border)] bg-[color:var(--bg-surface)] p-4 text-sm text-[color:var(--text-secondary)]">
                ...
                {invitation.status === "Accepted" && (
                  <Button
                    ...
                    onClick={() =>
                      navigate(
                        `/workspace/${invitation.workspaceId}/settings`,
                        { replace: true },
                      )
                    }
                  >
              ...
              <div className="mt-6 rounded-lg border border-[color:var(--border)] bg-[color:var(--bg-surface)] p-4 text-sm text-[color:var(--text-secondary)]">
                {isAuthenticated ? (
                  <p>
                    This invitation is assigned to{" "}
                    <span className="font-medium text-[color:var(--text-primary)]">
                      {invitationEmail}
                    </span>
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row">
                    <Link
                      to="/login"
                      state={{ from: location }}
                      className="inline-flex flex-1 items-center justify-center rounded-md bg-[color:var(--accent)] px-4 py-2.5 text-sm font-medium text-white hover:bg-[color:var(--accent-hover)]"
                    >
                    ...
                    <Link
                      to="/register"
                      state={{ from: location }}
                      className="inline-flex flex-1 items-center justify-center rounded-md border border-[color:var(--border)] px-4 py-2.5 text-sm font-medium text-[color:var(--text-primary)] hover:border-[color:var(--accent)]"
                    >
